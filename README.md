...
_The `on change` callback will return the items current state `checked` which is a bollean value where true = checked and false = unchecked along with the check radios container_ (`$element`) _and the real checkbox element_ (`$realElement`)_
...
Param 1: Returns boolean value based on whether the box is checked or unchecked. true = checked and false = unchecked.
Param 2: The jQuery object for the container generated by the plugin.
Param 3: The jQuery object for the actual element (checkbox | radio).
